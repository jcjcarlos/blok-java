...
public class MainPanel extends javax.swing.JPanel implements MouseListener, KeyListener {
	...
	public void bodiesUpdated(ArrayList<Body> bodies) {
		...
		for (Body body : bodies) {
			...
			if (body.getUserData() != null)
				// Player
				m_bodyRect.get(body).setLocation(size.width / 2 - 28 + (int) position.x,
						size.height / 2 - 28 - (int) position.y);
			else
				// Block
				m_bodyRect.get(body).setLocation(size.width / 2 - 14 + (int) position.x,
						size.height / 2 - 14 - (int) position.y);
		}
	...
	public void bodiesCreated2(ArrayList<AdapterPoint> points) {
		...
		for (AdapterPoint point : points) {
			...
			if (point.isPlayer()) {
				// Player
				rectangle.setRect(-28, -28, 56, 56);
				rectangle.setLocation(size.width / 2 - 28 + (int) point.getX(),
						size.height / 2 - 28 - (int) point.getY());
				m_player = rectangle;
			} else {
				// Block
				rectangle.setRect(-14, -14, 28, 28);
				rectangle.setLocation(size.width / 2 - 14 + (int) point.getX(),
						size.height / 2 - 14 - (int) point.getY());
			}
			adapter_pointRect.put(point, rectangle);
		}
		repaint();
	}
	...
	private HashMap<AdapterPoint, Rectangle> adapter_pointRect = new HashMap<AdapterPoint, Rectangle>();
	private Rectangle m_player;

	public enum State {
		INITIAL, RUNNING, YOUWON, YOULOST
	};
